...
for k, v in {"pid": None, "pname": None, "is_admin": False,
             "sel_group": "A", "balloons_done": False}.items():
    ss().setdefault(k, v)
...
def jersey_img(primary, secondary, pattern, size=64, cls="jersey-badge"):
    uri = avatar.data_uri(primary or "#2f81f7", secondary or "#ffffff",
                          pattern or "solid", size)
    return f'<img class="{cls}" src="{uri}" width="{size}" height="{size}" />'


def my_row():
    return conn.execute("SELECT * FROM participants WHERE participant_id=?",
                        (ss().pid,)).fetchone()
...
with st.sidebar:
    ...
    st.divider()
    page = st.radio("Go to", ["🏠 Home", "👤 My profile", "🎯 Match picks",
                              "🏆 Outcomes", "🃏 Wildcards", "📊 Leaderboard",
                              "🔐 Admin"], label_visibility="collapsed")
...
def lock_banner():
    if dbmod.predictions_locked(conn):
        st.markdown('<div class="lock-banner lock-on">🔒 Predictions are LOCKED by the organiser — viewing only.</div>',
                    unsafe_allow_html=True)
    else:
        st.markdown('<div class="lock-banner lock-off">✏️ Predictions are OPEN — you can edit until the organiser locks them.</div>',
                    unsafe_allow_html=True)
...
if page == "🏠 Home":
    ...
    lock_banner()
    if logged_in():
        nmatch = conn.execute("SELECT COUNT(*) FROM match_predictions WHERE participant_id=?",
                              (ss().pid,)).fetchone()[0]
        nout = conn.execute("SELECT COUNT(*) FROM outcome_predictions WHERE participant_id=?",
                            (ss().pid,)).fetchone()[0]
        nwild = conn.execute("SELECT COUNT(*) FROM wildcard_predictions WHERE participant_id=?",
                             (ss().pid,)).fetchone()[0]
        c1, c2, c3 = st.columns(3)
        c1.metric("Your match picks", f"{nmatch} / 104")
        c2.metric("Outcome picks", nout)
        c3.metric("Wildcard picks", f"{nwild} / 8")
        st.caption("Use the sidebar to navigate. Start with **My profile** to design your jersey!")
    else:
        need_login()
    ...
    if not logged_in():
        ...
        with col_form:
            ...
            ft = st.selectbox("Favorite team", teams,
                              index=teams.index(r["favorite_team"]) if r["favorite_team"] in teams else 0)
            ...
            sec = cc2.color_picker("Secondary colour", r["shirt_secondary"] or "#ffffff")
            pat = st.radio("Pattern", avatar.PATTERNS,
                           index=avatar.PATTERNS.index(r["shirt_pattern"])
                           if r["shirt_pattern"] in avatar.PATTERNS else 0,
                           horizontal=True)
        with col_preview:
            st.markdown("**Live preview**")
            st.markdown(f'<div style="text-align:center">{jersey_img(prim, sec, pat, 150)}</div>',
                        unsafe_allow_html=True)
        if st.button("💾 Save profile", type="primary"):
            dbmod.update_profile(conn, ss().pid, favorite_team=ft, favorite_player=fp,
                                 shirt_primary=prim, shirt_secondary=sec, shirt_pattern=pat)
...
# =========================================================================== #
    ...
    st.caption(f"Exact score = {config.MATCH_EXACT_SCORE} · correct goal diff = "
        ...
        if view == "Group stage":
            ...
            for rowg in rows:
                ...
                for i, g in enumerate(rowg):
                    ...
                    if cols[i].button(f"Group {g}", key=f"gbtn_{g}",
            ...
            matches = conn.execute(
                ...
                (g,)).fetchall()
            existing = {x["match_id"]: x for x in conn.execute(
            ...
            with st.form(f"mf_{g}"):
                ...
                for m in matches:
                    ...
                    hv = c2.number_input("H", 0, 30, value=ex["pred_home"] if ex else 0,
                                         key=f"h_{m['match_id']}", disabled=not can_edit,
                                         label_visibility="collapsed")
                    av = c3.number_input("A", 0, 30, value=ex["pred_away"] if ex else 0,
            ...
            ko_stages = [r["stage"] for r in conn.execute(
                "SELECT DISTINCT stage FROM matches WHERE is_knockout=1 ORDER BY match_id")]
            stage = st.selectbox("Round", ko_stages)
            matches = conn.execute("SELECT * FROM matches WHERE stage=? ORDER BY match_id",
                                   (stage,)).fetchall()
            existing = {x["match_id"]: x for x in conn.execute(
                "SELECT * FROM match_predictions WHERE participant_id=?", (pid,))}
            with st.form(f"kf_{stage}"):
                ...
                for m in matches:
                    ...
                    c4.markdown(f"{m['away_label']}")
                    picks.append((m["match_id"], hv, av))
                if st.form_submit_button(f"🔒 Lock in {stage} picks", type="primary",
...
# =========================================================================== #
    ...
    if not logged_in():
        ...
        with st.form("outcomes"):
            st.markdown(f"**Podium** — champion {OUTCOME_POINTS['champion']}, "
            ...
            sf = [pick(f"Semi-finalist {i}", "semi_finalist", str(i)) for i in range(1, 5)]
            st.markdown(f"**Golden Boot** ({OUTCOME_POINTS['golden_boot']} pts)")
            gb = st.text_input("Top scorer (player name)", value=ex.get(("golden_boot", ""), ""))
            ...
            for i, gc in enumerate([chr(c) for c in range(ord("A"), ord("L")+1)]):
                gteams = [x["name"] for x in conn.execute(
                ...
                gw[gc] = cols[i % 4].selectbox(f"Group {gc}", [""]+gteams, index=idx, key=f"gw_{gc}")
            if st.form_submit_button("🔒 Lock in outcome picks", type="primary"):
                rows = [("champion", "", champ), ("runner_up", "", runner),
                        ("third_place", "", third), ("finalist", "1", f1),
                        ("finalist", "2", f2), ("golden_boot", "", gb)]
            ...
            for w in conn.execute("SELECT * FROM wildcards ORDER BY wildcard_id"):
                ...
                if w["type"] == "number":
                    answers[w["wildcard_id"]] = st.number_input(
                        label, value=float(cur) if cur else 0.0, step=1.0, key=w["wildcard_id"])
                elif w["type"] in ("boolean", "choice"):
                    opts = [""] + w["options"].split("|")
                    idx = opts.index(cur) if cur in opts else 0
                    answers[w["wildcard_id"]] = st.selectbox(label, opts, index=idx, key=w["wildcard_id"])
                elif w["type"] == "team":
                    opts = [""] + teams
                    idx = opts.index(cur) if cur in opts else 0
                    answers[w["wildcard_id"]] = st.selectbox(label, opts, index=idx, key=w["wildcard_id"])
...
# =========================================================================== #
    ...
    st.markdown('<div class="wc-hero" style="padding:18px 26px"><div class="ball">🏆</div>'
                '<h1>Leaderboard</h1><p>The race for World Cup bragging rights.</p></div>',
                unsafe_allow_html=True)
    st.write("")
    rows = scoring.leaderboard_rows(conn)
    profiles = {r["participant_id"]: r for r in conn.execute("SELECT * FROM participants")}
    if not rows:
        ...
        for slot in order:
            ...
            jersey = jersey_img(pr["shirt_primary"] if pr else "#2f81f7",
                                pr["shirt_secondary"] if pr else "#fff",
                                pr["shirt_pattern"] if pr else "solid", 84)
            sparks = "".join(
                f'<span class="spark" style="left:{x}%;top:{y}%;animation-delay:{d}s"></span>'
                for x, y, d in [(15, 30, 0), (80, 20, .6), (50, 10, 1.1), (30, 60, .3)]
            ) if slot == 0 else ""
            html.append(
        ...
        st.dataframe(
            [{"#": r["rank"], "Player": r["name"], "Total": r["total_points"],
...
# =========================================================================== #
    ...
    with st.form("results"):
        ...
        for m in conn.execute("SELECT * FROM matches ORDER BY kickoff_utc, match_id"):
            ...
            hg = c2.number_input("H", 0, 30, value=cur["home_goals"] if cur else 0,
                                 key=f"rh_{m['match_id']}", label_visibility="collapsed")
            ag = c3.number_input("A", 0, 30, value=cur["away_goals"] if cur else 0,
                                 key=f"ra_{m['match_id']}", label_visibility="collapsed")
        ...
        with st.form("outres"):
            ...
            for i, gc in enumerate([chr(c) for c in range(ord("A"), ord("L")+1)]):
                gteams = [r["name"] for r in conn.execute(
                    "SELECT name FROM teams WHERE group_code=? ORDER BY name", (gc,))]
                gwres[gc] = gcols[i % 4].selectbox(f"Group {gc}", [""]+gteams, key=f"gwres_{gc}")
            if st.form_submit_button("Save outcome results"):
                ...
                for cat, ref, val in pairs:
                    if str(val).strip():
                        conn.execute("INSERT OR REPLACE INTO outcome_results VALUES (?,?,?)",
        ...
        with st.form("wres"):
            ...
            if st.form_submit_button("Save wildcard results"):
                for wid, val in vals.items():
                    if str(val).strip():
                        conn.execute("INSERT OR REPLACE INTO wildcard_results VALUES (?,?)",
                                     (wid, str(val).strip()))
                conn.commit()
                st.success("Wildcard results saved.")

    with st.expander("Reset a player's PIN"):
        pnames = [r["name"] for r in conn.execute("SELECT name FROM participants ORDER BY name")]
        if pnames:
            ...
            if st.button("Reset PIN") and newpin:
                pid = conn.execute("SELECT participant_id FROM participants WHERE name=?",
                                   (who,)).fetchone()[0]
